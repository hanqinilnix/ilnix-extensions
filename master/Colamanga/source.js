...
exports.ColamangaInfo = {
    ...
    icon: "icon.png",
    author: "hanqinilnix",
    description: "Paperback Extension for 漫画",
...
class Colamanga {
    ...
    CloudFlareError(status) {
        if (status == 503 || status == 403) {
            throw new Error(`CLOUDFLARE BYPASS ERROR:\nPlease go to the homepage of <${Colamanga.name}> and press the cloud icon.`);
        }
    }
    ...
    async getChapterDetails(mangaId, chapterId) {
        ...
        while (hasNextChapter.length > 0) {
            ...
            request = App.createRequest({
                url: `${COLAMANGA_URL}/${chapterId}_${++currentPageNumber}`,
                method: "GET",
            });
    ...
    async getSearchResults(query, metadata) {
        throw new Error("Method not implemented");
        let request = App.createRequest({
            url: `${COLAMANGA_URL}/search?q=${encodeURIComponent(query.title)}`,
            method: "GET",
        });
