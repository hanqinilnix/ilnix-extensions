...
export class Baozimh
    ...
    async getHomePageSections(
        ...
        popularSection.items = $(".index-rank > div > .comics-card")
            .toArray()
            .map(
                (manga: CheerioElement): PartialSourceManga =>
    ...
    async getMangaDetails(mangaId: string): Promise<SourceManga> {
        ...
        const images = $("amp-img")
            .toArray()
            .map(
                (element: CheerioElement): string => $(element).attr("src") as string
            );
    ...
    async getChapterDetails(
        ...
        const numOfImages: number = +$(".comic-text__amp")
            .last()
            .text()
            .trim()
            .split("/")[0]!;
        ...
        for (let i = 1; i <= numOfImages; i++) {
            samplePageUrl.pop();
            samplePageUrl.push(
                String(i) + ".jpg?t=" + Math.floor(Math.random() * Math.pow(2, 16))
            );
    ...
    async getSearchResults(
        ...
        let request = App.createRequest({
            url: `${BAOZIMH_URL}/search?q=${encodeURIComponent(
